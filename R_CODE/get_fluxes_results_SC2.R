...
require(ggmagnify)
dir <- "D:/PROGRAMAS/Dropbox/shared/Metabolic_network_manual_curation/GAMS/FLUX_RESULTS/SC2"
...
write.csv(x_scenarios,paste0("D:/PROGRAMAS/Dropbox/shared/Metabolic_network_manual_curation/GAMS/FLUX_RESULTS/SC2/results/","PSEUDO_JOINED_SC2_A.csv"),na = "",row.names = F)
...
x_scenarios_j_flux$scenario <- sub("200_S2_","",x_scenarios_j_flux$scenario)
...
x_scenarios_j_flux$interval <- rep(seq(0,1020,round((1020/500),3)),12*(length(rxn)+1))
...
outdir <- "D:/PROGRAMAS/Dropbox/shared/Metabolic_network_manual_curation/GAMS/FLUX_RESULTS/SC2/results"
#concs$colors <- P36

ggexport(
  x_lines1,
  filename = paste0(outdir,"/","PSEUDO_TEST_SC2_A",".pdf"),
  #width = 35,
  width = 20,
  #height = 600,
  #pointsize = 0.1,
...
ggexport(
  xx,
  filename = paste0(outdir,"/","PSEUDO_TEST_hormone_growth_SC2_A",".pdf"),
  #width = 35,
  width = 20,
  #height = 300,
  #pointsize = 0.1,
...
xxx <- x_scen_MERGE[which(x_scen_MERGE$variable==rxn_final_sc2[[3]]),]
plot_list[[3]]  <- plot_list[[3]] +   geom_magnify(aes(from=xxx$Interval>187 &
                                                         xxx$Interval<390
 & xxx$value>0 & xxx$value<0.0005),#c(208.08,399.84)),
                                                   to = c(xmin=80,xmax=150,
                                                          ymin=0.4,ymax=0.8),
# proj = "corresponding",
 shadow = TRUE
)
...
ggexport(
  figure3,
  filename = paste0(outdir,"/","fluxes_selected_sc2_final_A.pdf"),
  width = 20,
  height = 20,
  #pointsize = 0.1,
  res = 600,
...
ggexport(
  zoom_in,
  filename = paste0(outdir,"/","fluxes_selected_sc2_final_A_zoom_in.pdf"),
  width = 20,
  height = 20,
  #pointsize = 0.1,
  res = 600,
